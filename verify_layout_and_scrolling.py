...
def verify():
    with sync_playwright() as p:
        ...
        # Expectation: Height is FIXED (approx 2.9rem).
        # We also check that 'overflow: hidden' is NOT computed on .app (or is 'visible').
        # However, if content overflows, the page should handle it.
        # But wait, if .app is fixed height and overflow is visible, content will spill.
        # The user requested NO overflow: hidden.
        ...
        overflow = app.evaluate("el => window.getComputedStyle(el).overflow")
        print(f"Wide Desktop App Overflow: {overflow}")
        if overflow == 'hidden':
            print("FAIL: overflow: hidden is still present on .app in Wide Desktop.")
        else:
            print("PASS: overflow is not hidden.")

        # Check fixed height behavior
        h1 = app.evaluate("el => el.getBoundingClientRect().height")

        # Inject many divs to force expansion
        page_wide.evaluate("""
        ...
        if abs(h2 - h1) > 10:
            print("FAIL: Wide Desktop QuickLinks grew! Should be fixed height.")
        else:
            print("PASS: Wide Desktop QuickLinks height fixed.")

        # 2. Narrow Desktop Verification
        ...
        # Check for Body Scrolling when content is huge
        # We just added huge content. Let's see if body scrollHeight > clientHeight
        scroll_height = page_narrow.evaluate("document.body.scrollHeight")
        client_height = page_narrow.evaluate("window.innerHeight")
        print(f"Body Scroll Height: {scroll_height}, Window Height: {client_height}")

        if scroll_height > client_height:
             print("PASS: Body is scrolling.")
        else:
             print("FAIL: Body is NOT scrolling (content might be hidden or not enough).")
